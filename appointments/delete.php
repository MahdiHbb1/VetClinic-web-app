...
try {
    ...
    // Delete related records first (if appointment_layanan table exists)
    $stmt = $pdo->prepare("DELETE FROM appointment_layanan WHERE appointment_id = ?");
    $stmt->execute([$appointment_id]);

    // Update medical records to remove appointment reference
    $stmt = $pdo->prepare("UPDATE medical_record SET appointment_id = NULL WHERE appointment_id = ?");
    $stmt->execute([$appointment_id]);
